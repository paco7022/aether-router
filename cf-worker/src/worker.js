...
function getConfig(env) {
  return {
    // Kill switch for the home-PC origin. When PC_ENABLED="false" the worker
    // skips the PC probe entirely and routes straight to cloud — used after
    // decommissioning the PC so a powered-off box isn't probed (and possibly
    // double-fetched) on every request. Default: PC enabled.
    pcEnabled: String(env.PC_ENABLED ?? "true").trim().toLowerCase() !== "false",
...
export default {
  async fetch(request, env, ctx) {
    ...
    const inPcWindow = config.pcEnabled && isPcWindow(new Date(), config);
    ...
    if (inPcWindow && !circuitIsOpen()) {
      ...
      finalResponse = tagResponse(cloudRes, {
        "x-aether-origin": "cloud",
        "x-aether-routing-mode": !config.pcEnabled
          ? "cloud-only"
          : inPcWindow
          ? "day-circuit-open"
          : "night-cloud",
